...
*Version: **1.6 · APPROVED**, originally 21 August 2026, amended 22–26 August 2026 during construction · Branch: **React Web Application** · Stack: React 18 + Vite + TypeScript (strict), hosted on Vercel, push-to-deploy from `chrisbandre-web/fccprogramintegrator` · Status: **Released for construction***
...
- **Runtime:** Node **24.19.0**, pinned. Recorded exactly because Vercel builds on *its* Node version, not the Coordinator's, and a mismatch produces precisely the ambiguous failure DD §1 warns against. Pinned in three places that must agree: `.nvmrc`, `package.json` `engines.node`, and the Vercel project's Node.js Version setting.
  ...
  > 🔧 **Implementation Note — amended 22 August 2026, discovered in phase 1 construction.** Vercel's platform enforces Node version selection at **major-version granularity only** — its build log carries an explicit warning when `engines.node` names a full patch, and the dashboard's Node.js Version control likewise offers only a major-version choice (`24.x`). The exact patch pin above is not achievable on this host and is not a defect in the pin: it still governs local development (`nvm` and equivalent tools honour the full `24.19.0`) and it documents the intended version precisely, which is worth more than dropping to `24.x` everywhere. What changes is what phase 1's exit line checks for — see §B.3 and §J.1, both amended to check major-version match rather than exact patch, which is the only thing the platform can confirm.
...
- **Node parity is a phase 1 exit line:** the Vercel project's Node version must read 24.x and the deploy log must confirm it — **major-version match, or as close as the platform allows** (amended 22 Aug 2026; see §B.1's Implementation Note — Vercel does not support exact-patch selection, confirmed by its own build-log warning during phase 1 construction).
...
**The fresh-row rule, generically (⚠️ A8).** Walk the declarations in order; pack up to three elements per row; start a new row when `placement.group` differs from the previous element's; separate groups by `--register-group-gap` and rows within a group by `--register-row-gap`. **The shell never names a group and never counts one.** Group order is the order of first appearance in the registry, so re-ordering groups is re-ordering declarations.

> 📐 **Design Note — a single-element exception was tried and withdrawn *(23 August 2026, then reversed 26 August 2026)*.** A strict boundary break leaves *Recordkeeping* alone on a row of three. On 23 August the Coordinator ruled this should be avoided by merging the orphan with the opening element of the next group, reading the board as one continuous object rather than a partially-filled grid — that ruling is recorded above as "a future Engineer should not fix it back," which was accurate at the time. **On 26 August the same Coordinator reversed it:** the orphan row is preferable to *FCC Risk Appetite Statement* sitting apart from *Board Reporting* and *Risk Committee Reporting*, which the exception had produced by pulling Risk Appetite Statement into Recordkeeping's row instead of its own group's. **This is the second and current ruling; the exception is withdrawn, and a future Engineer should not reintroduce it without a fresh Coordinator instruction to do so** — the same caution the first ruling asked for, now pointed the other direction. The expected arrangement is enumerated at §L.4.5 and asserted mechanically by check **TAD-23** (§L.3), which was rewritten alongside this reversal rather than removed, so an accidental change away from either intended arrangement fails the run.
...
> 🔧 **Revised 28 Aug 2026 (Art Director note) — a plain-text caption on every row, always.** The 27 Aug revision above let `SegmentPills`' line-name pill stand in as the "header" for the recent-intake row, on the reasoning that a separate caption would just repeat the line name. That reasoning solved the wrong problem: the pill's line name answers *which line*, not *which of the two rows* — recent intake and book are a different fact, and only book was actually being named (recent intake had to be inferred from position). Worse for Whole Book specifically, since both its rows are interactive: both rendered a "Whole Book" pill, so the same name appeared twice with two different, seemingly contradictory sets of numbers. Signature moment two's entire argument is that recent intake differs from the established book — that comparison doesn't read if the two terms aren't both named. Every row on every group now carries a plain `RowCaption` — "Recent intake" above the upper bar, "Book" above the lower — in addition to `SegmentPills` wherever a row is interactive. Whole Book's book row keeps its pills (still selectable, per the ruling above that both its rows stay interactive) but is now captioned "Book" like every other line's book row, not "Whole Book" again — the caption was what was missing, not the interaction.
>
> 🔧 **Revised again 28 Aug 2026 (Coordinator, taking marketing screenshots) — the collapsed 30% allocation doesn't fit its own content, and that's now real rather than theoretical.** Computed directly: one collapsed row is caption (22.5 canvas) + gap (4) + pill row (26.5) + gap (4) + bar (24) = 81 canvas — up from 54.5 before captions came back two revisions above. Four rows plus three inter-section gaps plus `Comparison`'s own padding and its gap before `MethodologyLabel` plus `MethodologyLabel` itself comes to 508 canvas; the fixed `flex: '0 0 30%'` allocation of a 1016-canvas module height is 304.8 — short by a real, computed 203.2 canvas, not a rounding margin. Without containment this overflow doesn't clip or scroll, it spills straight into `RecordsPanel`'s own content below it (`CustomerIntakeModule.tsx`'s wrapper had `minHeight: 0` already, which stops the wrapper from refusing to shrink, but doesn't contain content taller than what it shrank to — the same two-part lesson as `RecordsPanel`'s own `overflow: hidden → auto` fix a day earlier). Fixed with `overflow: 'auto'` on the same wrapper, `tabIndex={-1}` for the same reason as `RecordsPanel`'s content div. This contains the overflow rather than eliminating the need for it — the collapsed comparison strip may still need to scroll internally to show all four lines within its 30% share, which the fix guarantees is safe (no more visual collision) but doesn't make invisible. Whether that's acceptable or whether the 30/70 split (or the collapsed-state row sizing) should be revisited is an open design question, not resolved here.
...
#### D.6.12 `SegmentPills` *(added 27 Aug 2026; revised 27 Aug 2026, Design System owner ruling, v1.4)*
...
- **Responsibility:** one `role="radiogroup"` of real `<button>`s — a neutral line-name pill (`rating: 'All'`) plus three rating pills (`High`/`Medium`/`Low`), each showing its own percentage. Satisfies Rule E (Touch Two Amendment 1 §3) in `CompositionBar`'s place. Every pill is a fixed-size click target regardless of the percentage it represents — the reason this exists: a 1%-wide bar segment is a few rendered pixels, not a reliable target, and the pill row solves that structurally rather than by widening tiny segments artificially.
- **Props / configuration:** `{ groupLabel, line, composition, state, dispatch }`.
- **State owned:** none.
- **Visual anatomy — Rule B (Touch One, unchanged), applied rather than invented: RAG is a mark, not a field.** Two prior iterations were tried and superseded before landing here — full-strength fill on every pill (competed visually with the bar), then a tinted-field version (rejected: `--risk-bar-low`'s source saturation, 0.09, has nothing left to dilute, and its hue angle goes numerically meaningless well before any usable tint ratio — no single ratio held across all three; §L.1 has the numbers).
  - **Rating selection has exactly one signal: fill vs. outline.** A solid pill against outlined pills is a *form* difference, not a colour one — it survives full desaturation (verified in greyscale, 27 Aug 2026), so it is already non-colour and already greyscale-safe without a second signal. **Regular weight throughout — do not reinstate bold as an accessibility measure. It never was one.** Fill alone carries the signal; bold was redundant with it, not reinforcing it. Selected-pill ink is Touch Two Amendment 1 §3's segment-label rule, reused verbatim: `--surface` on High (7.84:1 — the same ratio as `--risk-text-high` on `--surface`, the same colour pair with foreground and background swapped), `--ink-primary` on Medium and Low. The swatch is **omitted** on a selected pill ("the fill states it") and shown unconditionally otherwise, full-strength `--risk-bar-*`.
  - **Line selection is a separate signal, owned by `Comparison.tsx`, not this component:** the leading rule on the whole row (§D.6.5), present whenever the line is selected regardless of which rating is currently active within it. An earlier version of this component drew a leading-rule effect per pill, conflating a line-level signal with a rating-level one and duplicating a marker `Comparison.tsx` already drew correctly one level up. Removed.
  - The line-name ("All") pill extends the same fill-vs-outline rule when selected, with a neutral `--ink-primary` fill rather than a `--risk-bar-*` one — it has no associated risk colour. This extension is the Engineer's, not the Design System owner's; their ruling addressed the rating pills specifically.
  - The Low swatch sits at 1.74:1 against `--surface` — below the graphical floor, and accepted: the same value already governs the `MarkLegend` swatches (Amendment 2). Confirmed to hold in greyscale, including Low's, 27 Aug 2026 — the one the owner was least sure of going in.
- **Inputs / outputs:** in — a `Composition`. Out — `selectSegment`.
- **Zero-population case:** when `composition.total === 0`, renders "no records" (plain text, `--ink-tertiary`) instead of pills that would otherwise all show 0% and select nothing meaningfully different from each other.
- **Tokens:** `--pill-swatch-size` (12px, confirmed) and `--pill-keyline` (1px) — both v1.4 additions.
...
1. **Accessibility.** Keyboard operability of the core loop with the focus order fixed by DD §4: `Program Element → Time Horizon → About`, then on the Board `Customers tile`, and in the module — revised 27 Aug 2026, see §D.6.5/§D.6.12 — `each group's SegmentPills, top to bottom (Whole Book's Recent-intake pills, then Whole Book's Book pills, then each line's single pill row) → rating filter → close → pagination when present`. Recent intake precedes Book within Whole Book, matching the visual reorder (§D.6.5, 27 Aug 2026: recent-to-historical, recent intake now renders above book). Within a pill row, the line-name pill precedes the three rating pills, left to right. A business line's Book bar is display-only and contributes no stop. Focus rings are `--focus-ring` at `--focus-ring-offset` on every interactive element. **The 24 inactive elements are not focusable, which is itself the honesty signal.** Record rows are not focusable and the table is not a scroll region. **The collapsed Comparison wrapper is a scroll region as of 28 Aug 2026** (see §D.6.5's implementation), `tabIndex={-1}` for the same reason as `RecordsPanel`'s own content div — the pills inside it remain individually tabbable via ordinary DOM order regardless; only the wrapper itself is excluded from the tab sequence. The About modal is the only focus trap. All motion respects `prefers-reduced-motion`, which the token file implements by zeroing the three duration tokens — so honouring it requires no component to check anything.
...
- **Node parity:** the Vercel deploy log reads 24.x — major-version match, or as close as the platform allows (§B.1, §B.3; amended 22 Aug 2026, Vercel does not offer exact-patch selection).
...
| **`SegmentPills` swatch size** | §D.6.12 | **Closed, 27 Aug 2026.** Carried provisionally at 12 canvas pending a magnified screenshot of the rendered build, same convention as `--register-title-width`'s own provisional period (§F.10). Confirmed unchanged at 12px, and confirmed to hold in greyscale including Low's (the one the owner was least sure of, since it sits below the graphical floor). Landed as `--pill-swatch-size` in v1.4. |
| **Pill tint approach — rejected** | §D.6.12 | **Closed, 27 Aug 2026.** A tinted-field treatment (each rating pill filled with a lightened mix of its own `--risk-bar-*` value) was tried to quiet the pills against the bar's full saturation, at ~18% and ~28% mix ratios. Rejected: High's identity is carried by its darkness (source lightness 0.34; 0.86 at an 18% mix) — diluting it produces a different colour that shares a hue angle, not a lighter version of the same one ("shades of pink," accurately). Low is the decisive case: source saturation 0.09 leaves nothing to dilute, and its hue angle becomes numerically meaningless well before any usable ratio (90°–150° swing observed across the two ratios tried, against a 200° source). No single ratio holds for all three, so a tint scheme was abandoned in favour of the neutral-plus-swatch anatomy specified in `§D.6.12`. |
...
Walk declarations in registry order; pack up to three per row; break on group change — the plain strict break, with no exception (§D.5.16: a single-element merge exception was tried 23 August and withdrawn by a second Coordinator ruling 26 August). **The expected arrangement is enumerated rather than derived**, because it is the arrangement itself that is authoritative:
...
| 9 | Recordkeeping | Data, models and records — the orphan the withdrawn exception used to absorb |
| 10 | FCC Risk Appetite Statement · Board Reporting · Risk Committee Reporting | Governance and Reporting, together |

**Ten rows, 24 elements.** Check **TAD-23** asserts this table row by row.

> 🔧 **Implementation Note — why a total-row count is not the check, and how this document learned it.** Through v1.4 this section said *"if the packing does not produce ten rows, something is wrong before any pixel is inspected."* Both the exception arrangement and the plain strict-break arrangement produce ten rows while moving an element between them, so a total-only check cannot see the difference — and the vertical budget still fits either way. **A count is not an arrangement.** The assertion is therefore per-row membership, which is the property that actually matters — and it is why this table survived the 26 August reversal essentially unchanged in form, with only its ninth and tenth rows' content flipping back.

**Consequence for the vertical budget.** With the plain strict break (current, reverted 26 August): 570 (10 × 57) · four within-group gaps × 8 = 32 · five group separations × 24 = 120 · 96 · 24 · 140 = **982 against 1016, slack 34** — the original figure, restored, since the reverted algorithm is the same one phase 1 always used. (The single-element exception, while it stood between 23 and 26 August, merged one group-separation into a within-group gap and read 966/1016/slack 50 — a larger reserve that no longer applies now the exception is withdrawn.) `--tile-band-height` stands at 140 unless amended.
...
| **1.6** | **26 Aug 2026** | **Construction amendment, phase 6 — a second Coordinator reversal on the register arrangement.** Two changes, same day. **(a)** Engineer 3 found, during final phase 3–5 verification, that the 23 August single-element exception had never actually been implemented in `RegisterField.tsx` — only the plain strict break — despite being Coordinator-approved and despite v1.5's explicit "a future Engineer should not fix it back." The row *count* still read as ten either way, which is why it passed unnoticed; row 9–10 *content* was wrong. Implemented correctly and guarded by a rewritten check, **TAD-23** (v1.5's prose had called this check "TAD-22," which collided with the pre-existing HealthMark/scrim check of that literal ID — noted at the time, corrected here). **(b) Hours later, the Coordinator reversed the 23 August ruling itself:** the orphan row (*Recordkeeping* alone) is now preferred over *FCC Risk Appetite Statement* sitting apart from *Board Reporting* and *Risk Committee Reporting*, which the exception had produced. `RegisterField.tsx` reverted to the plain strict break; §D.5.16, §L.4.5 and check **TAD-23** all updated to match — the check rewritten in place rather than removed, so it now guards *against* reintroducing the exception rather than for it. The vertical-budget ledger reverts to 982/1016/slack 34, the same figure phase 1 originally confirmed. No component contract, phase or exit condition moves; this is the second word on a single layout question, and §D.5.16 records both rulings and their dates so a third would have the full history to work from. |
