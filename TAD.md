...
- **State owned:** none.
- **Visual anatomy — Rule B (Touch One, unchanged), applied rather than invented: RAG is a mark, not a field.** A first cut of this component filled every pill with its own `--risk-bar-*` at full strength regardless of selection, which visually competed with the bar beneath it; a tinted-field alternative was tried and rejected (§L.1 — Low's source saturation, 0.09, has nothing left to dilute, and its hue angle goes numerically meaningless well before any usable tint ratio, so no single ratio works across all three).
  - **Unselected:** `--surface` fill, 1px `--surface-edge` keyline, `--ink-primary` text, a small square swatch in the leading position at the segment's own `--risk-bar-*` value, full strength.
  - **Selected:** fills with its own `--risk-bar-*` at full strength, semibold weight, a leading rule — three signals, so selection is never colour alone. Ink is Touch Two Amendment 1 §3's segment-label rule, reused verbatim rather than re-specified: `--surface` on High, `--ink-primary` on Medium and Low.
  - The Low swatch sits at 1.74:1 against `--surface` — below the graphical floor, and accepted: the same value already governs the `MarkLegend` swatches (Amendment 2), and Rule E is satisfied by the word "Low" beside it, not by the swatch's own contrast.
...
- **One token not yet confirmed:** the swatch's own size, carried provisionally at **12 canvas**, same convention as `--register-title-width`'s own provisional period (§L.1, §F.10) — requested as a Touch Two v1.4 addition once a magnified screenshot of the rendered build confirms it, rather than issued from arithmetic.
...
| **`SegmentPills` swatch size** | §D.6.12 | **Open, flagged 27 Aug 2026 (Design System owner).** Carried provisionally at 12 canvas — the owner asked to confirm this against a magnified screenshot of the rendered build rather than issue it from arithmetic, same convention as `--register-title-width`'s own provisional period (§F.10). Requested as a Touch Two v1.4 addition once confirmed. |
